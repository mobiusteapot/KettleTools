...
namespace ETC.KettleTools.Audio.FMODAddon{
    ...
    {
        // Should derive this from it's scriptable object
        [SerializeField]
        ...
        [System.Obsolete("Use the EventReference field instead")]
        public string Event = "";
        ...
        private static List<KTEventEmitter> activeEmitters = new List<KTEventEmitter>();
        ...
        {
            foreach (KTEventEmitter emitter in activeEmitters)
            {
                emitter.UpdatePlayingStatus();
            }
        }

        private static void RegisterActiveEmitter(KTEventEmitter emitter)
        {
        ...
        private static void DeregisterActiveEmitter(KTEventEmitter emitter)
        {
        ...
        #region custom KT methods
        public void OnValidate() {
            if (eventAudioContainer != null)
            {
                EventReference = eventAudioContainer.GetAudioBundleByIndex(0).GetAudio();
            }
        }

        public void PlayEventByIndex(int index) {
            EventReference = eventAudioContainer.GetAudioBundleByIndex(index).GetAudio();
            Play();
        }
        public void StopEventByIndex(int index) {
            EventReference = eventAudioContainer.GetAudioBundleByIndex(index).GetAudio();
            Stop();
        }
        ...
        public void SetParameterByEventIndex(int index, FMOD.Studio.PARAMETER_ID id, float value, bool ignoreseekspeed = false) {
            EventReference = eventAudioContainer.GetAudioBundleByIndex(index).GetAudio();
            SetParameter(id, value, ignoreseekspeed);
        }
#endregion
